...
def test_parse_by_unstructured(mocker, unstructured_pdf_parser):
    ...
    mocker.patch.object(
        unstructured_pdf_parser,
        "_prepare_unstructured_data_to_e2m_parsed_data",
...
def test_parse_by_surya_layout(mocker, surya_layout_pdf_parser):
    ...
    mocker.patch.object(
        surya_layout_pdf_parser, "surya_layout_func", return_value=[{"name": "image1"}]
    )
    mocker.patch.object(
        surya_layout_pdf_parser,
        "_prepare_surya_layout_data_to_e2m_parsed_data",
        return_value=E2MParsedData(),
    )

    parsed_data = surya_layout_pdf_parser._parse_by_surya_layout(file=test_pdf_path)

    mock_convert_pdf_to_images.assert_called_once_with(
        test_pdf_path, None, None, 1, save_dir="./.tmp", dpi=180
    )
...
def test_get_parsed_data_unstructured(mocker, unstructured_pdf_parser):
    mocker.patch.object(
        unstructured_pdf_parser, "_parse_by_unstructured", return_value=E2MParsedData()
    )
...
def test_get_parsed_data_surya_layout(mocker, surya_layout_pdf_parser):
    mocker.patch.object(
        surya_layout_pdf_parser, "_parse_by_surya_layout", return_value=E2MParsedData()
    )
